...
Firebase App Hosting へのデプロイを試行し、Googleログイン自体は承認済みドメイン設定により動作確認できました。一方で、Todo の取得 API 実行時にサーバーエラーが発生し、ローカル開発で使用していた SQLite 構成をそのまま本番環境で運用する難しさを確認しました。

今回の実装では学習効率を優先して SQLite を採用しましたが、今後は本番公開を前提に、 PostgreSQL などの外部DBを利用した構成に改善したいと思います。
...
- テストコードの追加
- UI / UX の改善
- SQLite から本番運用を前提とした外部DBへの移行
- Firebase App Hosting での公開対応の再検証
